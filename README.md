...
- **更新检查失败提示** — 手动检查更新时区分"有新版本 / 已是最新 / 检查失败"，网络异常、超时或服务器错误不再被误报为"已是最新"
- **图书馆去重** — 分页加载、缓存恢复、合并全过程按 id 去重，冷启动不再累积重复书籍
- **播放历史详情本地化** — 暂停原因、本地直连、恢复、卡顿恢复、直达文件等事件详情词全部接入中英文语言包
...
<details>
<summary><b>展开查看完整更新日志 (v1.11.2+280 → v1.21.5+370)</b></summary>

### v1.21.5+370 — 更新检查反馈与本地化补全

- 更新检查三态结果：区分"有新版本 / 已是最新 / 检查失败"，设置页与启动检查在网络异常、超时或服务器错误时明确提示"无法检查更新"，不再静默当作已是最新
- 章节跳转锚定修复：上一章/下一章按待跳转目标位置解析章节，跨会话升级（每章独立音频文件）时不再因流位置塌缩而误跳回第一章
- 跳章标题防覆盖：快速连续跳章时，被更新的跳转覆盖的旧 seek 不再把陈旧（常为最后一章）标题盖回
- 片头跳过钉住章节：auto-skip 预跳直接钉住目标章节，消除引擎切轨瞬间显示错误章节标题的闪动
- 图书馆重复条目修复：分页/缓存恢复/合并按 id 去重，冷启动不再累积重复书
- 播放历史详情本地化：暂停原因（离线/误报完成拦截/iOS 提前完成拦截）、本地直连、恢复、卡顿恢复、直达文件、完成、睡眠回退撤销等事件详情词全部接入中英文语言包
